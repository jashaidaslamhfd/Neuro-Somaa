...
from intelligence import truth_gate as tg  # noqa: E402
...
class TestCalibration(unittest.TestCase):
    def _hist(self, pairs):
        return [{"hook_score": p, "views": v, "average_view_percentage": r}
                for p, v, r in pairs]
    ...
    def test_uncalibrated_metric_is_locked_out(self):
        ...
        pairs = [(70 + (i % 5) * 10, max(3, 1400 - i * 100), 35.0)
                 for i in range(14)]
    ...
    def test_bias_measured(self):
        hist = [{"hook_score": 80, "views": 500, "average_view_percentage": 38.0,
                 "predicted_retention": 0.70} for _ in range(15)]
...
class TestEmpiricalPrediction(unittest.TestCase):
    def test_similar_videos_drive_the_prior(self):
        ...
        self.assertTrue(all("ventre" in t.lower() or "peur" in t.lower()
                            for t in pred["similar"]))

    def test_unknown_topic_falls_back_honestly(self):
        hist = [{"topic": "xyz", "title": "Pourquoi xyz arrive ?", "views": 500,
                 "average_view_percentage": 40.0} for _ in range(20)]
...
class TestStatusFile(unittest.TestCase):
    ...
    def test_status_roundtrip(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            hist = [{"hook_score": 80, "views": 100 * i,
                     "average_view_percentage": 40.0} for i in range(1, 16)]
            out = tg.run(hist, p)
            self.assertTrue(p.exists())
            loaded = tg.load_status(p)
            self.assertIsNotNone(loaded)
            self.assertEqual(loaded["hook_score"]["verdict"],
                             out["status"]["hook_score"]["verdict"])
...
class TestLiveDataTruth(unittest.TestCase):
    ...
    def test_real_scores_are_exposed(self):
        ...
        self.assertGreater(cal["predicted_retention"]["bias"], 0.15,
