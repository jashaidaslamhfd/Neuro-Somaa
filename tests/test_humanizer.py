...
class TestRegister(unittest.TestCase):
    ...
    def test_cela_and_nous(self):
        self.assertEqual(self.h("Cela vous arrive quand vous dormez."), "Ça t'arrive quand tu dors.")
        self.assertEqual(self.h("Nous allons voir pourquoi."), "On va voir pourquoi.")

    def test_gendered_possessives(self):
        self.assertEqual(
            self.h("Votre peau se fripe, votre corps aussi."), "Ta peau se fripe, ton corps aussi."
        )
        self.assertEqual(self.h("Vos mains deviennent moites."), "Tes mains deviennent moites.")

    def test_imperatives(self):
        self.assertEqual(self.h("Imaginez votre cerveau la nuit."), "Imagine ton cerveau la nuit.")
        self.assertEqual(self.h("Abonnez-vous pour la suite."), "Abonne-toi pour la suite.")

    def test_nest_ce_pas(self):
        self.assertEqual(self.h("C'est étrange, n'est-ce pas ?"), "C'est étrange non ?")

    def test_cliche_drop(self):
        self.assertEqual(self.h("Il est important de noter que le corps se fige."), "Le corps se fige.")
...
class TestPipelineWiring(unittest.TestCase):
    def test_normalize_scenes_repairs_short_hook_for_opening_density(self):
        ...
        data = {
            ...
            "scenes": [
                {"visual": "muscle", "caption": "Pourquoi ton muscle tressaille tout seul ?"},
                {"visual": "nerf", "caption": "C'est un réflexe du système nerveux."},
            ],
        }
        out = script_generator._normalize_scenes(data)
        self.assertEqual(
            out["scenes"][0]["caption"],
            "Pourquoi ton muscle tressaille tout seul en ce moment?",
        )
        self.assertEqual(out["hook"], out["scenes"][0]["caption"])
        self.assertEqual(len(out["hook"].split()), 9)
    ...
    def test_normalize_scenes_repairs_inline_conjunction(self):
        ...
        out = script_generator._normalize_scenes(data)
        self.assertEqual(out["scenes"][0]["caption"], "Le signal arrive. Le muscle repart en ce moment.")
    ...
    def test_normalize_scenes_humanizes(self):
        ...
        data = {
            ...
            "scenes": [
                {"visual": "gros plan poitrine", "caption": "Vous entendez votre cœur la nuit ?"},
                {"visual": "illustration cœur", "caption": "C'est ton corps qui baisse le rythme."},
