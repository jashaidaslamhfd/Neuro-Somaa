...
class FranceChannelTests(unittest.TestCase):
    ...
    def test_long_angle_uses_catalogue_question_before_secondary_titles(self):
        ...
        ranked = generate_ab_variants(
            {
                "title": package["chosen_title"],
        ...
        self.assertEqual(ranked["recommended"]["title"], "Pourquoi la chair de poule apparaît soudainement ?")
        ...
        old_broken = generate_seo_package(
            ...
            {
                "series_title": "Battements entendus",
                "title": "Battements entendus",
        ...
        for sample in (
            "Ce qu\u2019il faut comprendre sur le visage qui rougit par gêne",
            "Pourquoi le cerveau remarque entendre son cœur battre la nuit",
        ):
    ...
    def test_gate_blocks_broken_french_voiceover(self):
        ...
        broken = {
            "title": "Battements entendus",
            "hook": "On entend son cœur la nuit",
            "cta": "Abonnez-vous pour plus de science.",
            "description": "Le calme aide à entendre son corps.",
            "topic": "les battements de cœur entendus la nuit",
            "scenes": [
                {"caption": c, "visual": "un corps humain dans la nuit"}
                for c in [
    ...
    def test_gate_approves_clean_french_script(self):
        ...
        clean = {
            "title": "Battements entendus",
            "hook": "Vous entendez votre cœur la nuit ?",
            "cta": "Abonnez-vous pour plus de science simple.",
            "description": "Le calme laisse mieux entendre le corps.",
            "topic": "les battements de cœur entendus la nuit",
            "scenes": [
                {"caption": c, "visual": "un coeur humain dans la nuit"}
                for c in [
