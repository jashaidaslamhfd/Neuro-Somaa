...
def _video(title: str, views: int, hook: float = 70.0, day: int = 1, hour: int = 19,
           retention: float = 40.0) -> dict:
    return {
...
class FeatureTests(unittest.TestCase):
    ...
    def test_dataset_only_uses_real_views(self):
        from intelligence import features
        rows, y, ids = features.build_dataset([
            _video("A ok ?", 300), {"title": "ghost", "views": None},
        ])
    ...
    def test_topic_bucket_stable_across_imports(self):
        ...
        self.assertEqual([a[k] for k in a if k.startswith("topic_bucket")],
...
class TinyMLPTests(unittest.TestCase):
    def test_mlp_fits_better_than_constant(self):
        ...
        mae_model = sum(abs(p - t) for p, t in zip(preds, true)) / len(true)
...
class StatsTests(unittest.TestCase):
    ...
    def test_permutation_honest_about_equal_arms(self):
        from intelligence import stats
        out = stats.permutation_test([700.0, 720, 680, 710] * 2,
                                     [705.0, 695, 715, 700] * 2, iters=2000)
