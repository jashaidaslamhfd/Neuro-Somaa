...
class AntiSpamSystem:
    ...
    def _check_duplicates(self, video: dict, previous_videos: list[dict]) -> list[str]:
        ...
        for prev_video in previous_videos:  # full channel history
            ...
            if current_title == prev_title:
                issues.append(f"🚨 CRITICAL: Duplicate title found: '{current_title}'")
                ...
                issues.append(f"⚠️ WARNING: Very similar title to: '{prev_title}'")
    ...
    def _get_spam_recommendation(self, risk_level: str, risks: list[str]) -> str:
        ...
        """
        recommendations = {
            "CRITICAL": "🔴 DO NOT PUBLISH - High spam risk. Rewrite content significantly.",
            "HIGH": "🟠 CAUTION - Address issues before publishing",
            "MEDIUM": "🟡 REVIEW - Minor issues detected, consider improvements",
        }
        return recommendations.get(risk_level, "🟢 SAFE - Low spam risk. Can publish.")
