...
MIN_HOOK_SCORE = int(os.environ.get("MIN_HOOK_SCORE", "0"))
...
# EXACT DUPLICATE BLOCK — prevents same title repetition (image evidence)
def _block_exact_duplicate_title(current_title: str, video_history: list) -> bool:
    for hist in video_history:
        hist_title = hist.get("title", "")
        if _normalize_title_key(current_title) == _normalize_title_key(hist_title):
            return True  # Block exact duplicate
    return False

def _near_duplicate_title(left: str, right: str, threshold: float = 0.50) -> bool:
    ...
    def generate_with_niche_strategy(
        ...
        for attempt in range(1, MAX_SCRIPT_ATTEMPTS + 1):
            try:
                ...
                # EXACT DUPLICATE BLOCK — image evidence: same title repeated
                if self.video_history:
                    for hist in self.video_history:
                        if str(current_topic).lower() == str(hist.get("topic", "")).lower():
                            logger.error(f"DUPLICATE TOPIC BLOCKED: {current_topic} (same as previous video)")
                            # Force different selection by modifying topic slightly or retrying
                            current_topic = current_topic + " (variante)"
                            break
...
def _exact_duplicate_title_check(left: str, right: str) -> bool:
    """Block exact same normalized title (prevents repetition like image shows)."""
    return _normalize_title_key(left) == _normalize_title_key(right)
