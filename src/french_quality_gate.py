...
FORBIDDEN_MEDICAL_PATTERNS = [
    r"\bgu[eé]rit\b",
    r"\bgu[eé]rison garantie\b",
    r"\brem[eè]de miracle\b",
    r"\bdiagnostique\b",
    r"\btu as (un|une|la|le)\b",
    r"\btu souffres de\b",
    r"\barr[eê]te (ton|tes|le|la) traitement\b",
    r"\bremplace ton traitement\b",
    r"\bpas besoin de m[eé]decin\b",
    r"\bignore ton m[eé]decin\b",
    r"\b100\s*%\s*(garanti|efficace|vrai)\b",
    r"\bpreuve absolue\b",
    # English slips
    r"\bcures?\b",
    r"\bdiagnos(e|is)\b",
    r"\bguaranteed\b",
    r"\bdoctor[s]? don't want\b",
]
...
BROKEN_CONTINUATION = re.compile(r"[.!?…]\s+(et|ou|ni)\b", re.IGNORECASE)
...
def _broken_narration_issues(scenes: list) -> list[str]:
    ...
    if match:
        issues.append(
            f"Broken French continuation after a full stop: {joined[max(0, match.start() - 25) : match.end() + 25]!r}"
        )
