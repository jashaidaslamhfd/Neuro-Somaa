...
BODY_KEYWORDS_FR = {
    "cerveau", "corps", "sommeil", "coeur", "cœur", "ventre", "muscle", 
    "nerf", "frisson", "bâillement", "bâille", "yeux", "oeil", "œil", 
    "peau", "gorge", "fatigue", "estomac", "mémoire", "stress", "tête"
}
...
def score_topic_fr(topic: str, source: str, views: int = 0) -> dict:
    ...
    if lowered.startswith("pourquoi votre") or lowered.startswith("pourquoi le") or lowered.startswith("pourquoi on"):
        score += 15
    elif lowered.startswith("ce qui se passe quand") or lowered.startswith("ce que votre corps"):
        score += 10
    elif lowered.startswith("la science derrière"):
        score += 10
...
def get_hijacked_viral_topic_fr(exclude_list: list[str] = None) -> dict:
    """
    ...
    for cand in candidates:
        ...
        if (topic_normalized not in seen and 
            ...
            any(kw in topic_normalized for kw in BODY_KEYWORDS_FR)):
    ...
    logger.info(f"🏆 French Winner Chosen: '{winner['topic']}' | Source: {winner['source']} | Score: {winner['score']}/100")
