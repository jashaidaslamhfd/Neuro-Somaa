...
def build_markdown(report: dict) -> str:
    ...
    growth_rows = [
        (e.get("growth_state"), e.get("views_per_day"), e.get("title") or e.get("topic") or "?")
        for e in hist
        if e.get("growth_state")
    ]
    if growth_rows:
        ...
        for _state, vel, title in fastest:
    ...
    if models.get("cv_r2_mean") is not None:
        lines.append(
            f"- ridge log-vues: **R²_cv = {models['cv_r2_mean']} ± {models['cv_r2_std']}** (MAE ≈ {models['cv_mae_views']} vues) — {'✅ fiable' if models.get('reliable') else '⚠️ bruit, conseils seulement'}"
        )
    ...
    lines.append(
        f"- MLP: {'MAE ≈ ' + str(mlp.get('in_sample_mae_views')) + ' vues (advisory)' if mlp.get('in_sample_mae_views') else mlp.get('reason', 'n/a')}"
    )
    ...
    if rec:
        lines.append(
            f"- ✅ pattern recommandé: **{rec['pattern']}** (winner-rate {rec['thompson_score']}, avg {rec['avg_views']} vues)"
        )
    for name, arm in list(bandit.get("arms", {}).items())[:5]:
        badge = "✅" if arm["confident"] else "🔬"
        lines.append(
            f"  {badge} `{name:12s}` n={arm['n']:>2} · winner-rate {arm['winner_rate']:.0%} · avg {arm['avg_views']}"
        )

    lines += (
        ["", f"## 🚨 Anomalies ({anoms.get('method', 'n/a')})"]
        if anoms.get("reliable")
        else ["", f"## 🚨 Anomalies — {anoms.get('reason', 'n/a')}"]
    )
    ...
    if fc.get("reliable"):
        ...
        lines.append(
            f"- tendance: **{fc['daily_trend']:+.1f} vues/jour^²** · attendu 30j: **{fc['total_30d_expected']:.0f} vues** (bande {fc['band_30d_low']}–{fc['band_30d_high']}/j)"
        )
    else:
    ...
    if clusters.get("reliable"):
        lines += [
            "",
            f"## 🗂️ Clusters de sujets (k={clusters['k']}) — gagnant: **{clusters['winner_cluster']}**",
        ]
        for c in clusters["clusters"][:5]:
            lines.append(
                f"- `{c['name']}` — {c['size']} vidéos · avg {c['avg_views']} vues · max {c['max_views']}"
            )
    ...
    if exp.get("available"):
        verdict = (
            f"**{exp['winner']}** gagne" if exp.get("winner") else "pas de différence significative encore"
        )
        lines += [
            "",
            f"## 🧪 Expérience durée: {verdict} (p={exp.get('p_value')})",
    ...
    if ha.get("available"):
        ...
        lines += [
            "",
            f"## 🪝 Expérience hooks — leader actuel: **{lead.get('arm')}** (avg {lead.get('avg_views')} vues)",
        ]
