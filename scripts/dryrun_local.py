...
SCRIPT = {
    ...
    "scenes": [
        ...
        {"visual": "doigts froissés dans un bain", "caption": "Ce n'est pas l'eau qui entre dans la peau, c'est une réaction nerveuse."},
        {"visual": "vaisseaux sanguins de la main", "caption": "Le système nerveux resserre les vaisseaux des doigts."},
        {"visual": "science du corps humain", "caption": "Ce réflexe améliore la prise des objets mouillés."},
        {"visual": "cerveau et nerfs", "caption": "Une adaptation ancienne que le corps garde encore aujourd'hui."},
...
def main():
    ...
    seo = check("seo_generator", lambda: __import__("seo_generator").generate_seo_package(
        SCRIPT["topic"], SCRIPT))
    if seo:
    ...
    imgs = check("image_generator", lambda: [
    ...
    segs = check("voice_generator", lambda: __import__("voice_generator").generate_voice_segments(
    ...
    # Phase 4: build video
    out = check("video_editor.build_video", lambda:
        __import__("video_editor").build_video(img_paths, segs, SCRIPT["scenes"],
        output_path="output/dryrun_final.mp4"))
    ...
    # Quality gates
    print("\n--- QUALITY GATES ---")
    check("french_quality_gate", lambda:
        __import__("french_quality_gate").validate_publication_quality(SCRIPT))
    check("quality_checker", lambda:
        __import__("quality_checker").QualityChecker().check_script_quality(SCRIPT))
    check("anti_spam", lambda:
        __import__("anti_spam").AntiSpamSystem().check_script_for_spam(SCRIPT))
    check("final_video_audit", lambda:
        __import__("final_video_audit").run_final_publication_audit(
