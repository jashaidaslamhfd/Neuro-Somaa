...
_2026-08-25T05:59:25.566591+00:00_ — n=67 vidéos réelles
...
- 🟢 en croissance: **5** · 🔴 figées (2 lectures sans hausse): **59**
  · ⚡ +26.7 vues/j — Pourquoi le hoquet ne s'arrête pas ?
  · ⚡ +2.2 vues/j — Pourquoi la main s'engourdit quand on dort dessus 
  · ⚡ +1.4 vues/j — Pourquoi le corps gratte après la douche ?
...
- ridge log-vues: **R²_cv = 0.4353 ± 0.164** (MAE ≈ 419.0 vues) — ✅ fiable
- facteurs dominants: `is_question` (+), `hour_sin` (-), `has_second_person` (-), `hour_cos` (+), `phrase_words` (-)
- MLP: MAE ≈ 260.4 vues (advisory)

## 🎰 Bandit de titres (Thompson sampling)
- ✅ pattern recommandé: **POURQUOI** (winner-rate 0.1668, avg 604.4 vues)
  🔬 `CE_QUE_VOTRE_CORPS` n= 1 · winner-rate 0% · avg 2.0
  🔬 `CE_QUIL_FAUT_COMPRENDRE` n= 1 · winner-rate 0% · avg 3.0
...
- tendance: **-115.0 vues/jour^²** · attendu 30j: **71 vues** (bande 0.0–564.7/j)
...
- `raison / change / dit / cerveau / vous` — 32 vidéos · avg 600.8 vues · max 1134
- `cœur / battre / son / nuit / sous` — 4 vidéos · avg 572.5 vues · max 874
...
## 🧪 Expérience durée: pas de différence significative encore (p=0.3255)
- `control_long` avg 765.25 vs `test_short` avg 912.56 (n=8/9)

## 🪝 Expérience hooks — leader actuel: **pov_reveal** (avg 635.7 vues)
- `pov_reveal` (635.73) vs `question` (361.8) — p=0.2394 (ns)
