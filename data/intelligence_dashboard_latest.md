...
_2026-08-27T16:40:02.422639+00:00_ — n=67 vidéos réelles
...
- 🟢 en croissance: **1** · 🔴 figées (2 lectures sans hausse): **65**
  · ⚡ +1.4 vues/j — Pourquoi le hoquet ne s'arrête pas ?
  · ⚡ +-0.7 vues/j — Pourquoi le corps révèle le mensonge ?
  · ⚡ +-32.0 vues/j — Réveil avant l'alarme

## 🧪 Truth Gate (scores internes vs réalité)
- 🔴 `hook_score` (hook quality): **NOISE** (r=-0.03, n=67) — **consultatif seulement, jamais un gate**
- 🟡 `seo_score` (SEO quality): **WEAK** (r=+0.27, n=67) — peut guider des décisions
- 🔴 `predicted_retention` (retention): **NOISE** (r=-0.09, n=67) — biais +0.24 (prédit 0.69 vs réel 0.45) — **consultatif seulement, jamais un gate**
- 🟡 `predicted_ctr` (CTR): **WEAK** (r=+0.26, n=67) — peut guider des décisions

## 🤖 Modèles (ridge + MLP, validation croisée)
- ridge log-vues: **R²_cv = 0.3969 ± 0.1634** (MAE ≈ 410.4 vues) — ✅ fiable
- facteurs dominants: `is_question` (+), `hour_sin` (-), `hour_cos` (+), `has_second_person` (-), `seo_score` (+)
- MLP: MAE ≈ 262.1 vues (advisory)

## 🎰 Bandit de titres (Thompson sampling)
- ✅ pattern recommandé: **POURQUOI** (winner-rate 0.1501, avg 584.0 vues)
...
- 🔻 **Pourquoi l'estomac gargouille quand on a faim ?** — 1 vues (z=-8.09): _queue metadata+thumbnail repair (hooks, CTR signals)_
- 🔻 **Pourquoi la musique change l'humeur ?** — 36 vues (z=-4.01): _queue metadata+thumbnail repair (hooks, CTR signals)_

## 📈 Prévision 30 jours (Holt)
- tendance: **-112.1 vues/jour^²** · attendu 30j: **91 vues** (bande 0.0–555.6/j)

## 🗂️ Clusters de sujets (k=6) — gagnant: **seul / tout / ventre / muscle / pied**
- `seul / tout / ventre / muscle / pied` — 3 vidéos · avg 1019.7 vues · max 1168
- `raison / change / dit / cerveau / vous` — 32 vidéos · avg 589.6 vues · max 1134
- `cœur / battre / son / nuit / sous` — 4 vidéos · avg 572.8 vues · max 874
- `corps / s'endormant / après / lourd / refuse` — 8 vidéos · avg 538.0 vues · max 986
- `froid / sembler / peut / étrange / frissonne` — 7 vidéos · avg 416.3 vues · max 1241

## ⏱️ Rétention
- P10/P50/P90 = 29.1% / 43.3% / 65.6% · **73%** des vidéos perdent le spectateur avant la moitié

## 🪝 Expérience hooks — leader actuel: **pov_reveal** (avg 635.6 vues)
- `pov_reveal` (635.64) vs `question` (368.8) — p=0.2484 (ns)
